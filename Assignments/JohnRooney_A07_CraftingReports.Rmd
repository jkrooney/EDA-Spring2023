...
| Metadata Attribute Name | Metadata Value                      |
|-------------------------:|:-------------------------------------|
...
```{r project.setup, include = F, message = F, warning = F}
#Import libraries
...
kable(summary_PM25,col.names = c("County","$\\mu g/m^3$"),caption = "Mean Particulates (2.5)")
...
```{r 2018 Ozone, echo=F, warning=F, fig.align='left', fig.cap='Ozone Levels by Month in 2018',fig.height=2.75}
Ozone.2018 <- epa_data %>%
  filter(Year=="2018")

ggplot(Ozone.2018, aes(x=factor(Month,level=1:12,labels=month.abb),y=Ozone)) +
         geom_boxplot() +
         labs(
           x="Month",
           y="Ozone")
...
```{r 2019 Ozone, echo=F, warning=F, fig.align='left', fig.cap='Ozone Levels by Month in 2019',fig.height=2.75}
Ozone.2019 <- epa_data %>%
  filter(Year=="2019")

ggplot(Ozone.2019, aes(x=factor(Month,level=1:12,labels=month.abb),y=Ozone)) +
         geom_boxplot() +
         labs(
           x="Month",
           y="Ozone")
...
[My Git Repository](https://github.com/jkrooney/EDA-Spring2023)
