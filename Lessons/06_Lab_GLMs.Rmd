...
regression1 <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(regression1)
...
library(corrplot)
EPAair_variables <- EPAair %>%
  select(mean_Lat:Ozone) %>%
  na.omit()

EPAairCorrelation <- cor(EPAair_variables)
corrplot(EPAairCorrelation, method = "ellipse")
...
EPAair.naomit <- EPAair %>%
  na.omit()

EPA_AIC <-lm(data = EPAair.naomit, PM2.5 ~ Ozone + Year + Month + mean_Lng + mean_Lat)
step(EPA_AIC)

#Exercise 6: Run another regression using the variables selected on Exercise 5. Compare r-squared value with the one from Exercise 2. 
OzoneBest.regression <- lm(data=EPAair.naomit, PM2.5 ~ Ozone + Month + Year + mean_Lng + mean_Lat)
